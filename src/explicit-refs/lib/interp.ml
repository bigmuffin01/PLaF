...
let rec eval_expr : expr -> exp_val ea_result = fun e ->
  ...
  | Proj(e,id) ->
    ...
    (match (List.assoc id n) with
    | (true, RefVal v) ->
      Store.deref g_store v
    | (false, v) -> return v)
  | SetField(e1,id,e2) ->
    eval_expr e1 >>=
    fields_of_recordVal >>= fun n1 ->
    eval_expr e2 >>= fun n2 ->
    (match (List.assoc id n1) with
    | (true, RefVal v) ->
      Store.set_ref g_store v n2 >>= fun _ ->
      return UnitVal
    | (false, v) -> error "Field not mutable")
